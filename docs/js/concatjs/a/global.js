function AllFunc() {
	...
	_all_func.prototype = {
		...
		MyGet: function (url, suc, data, noLoad) {
			var that = this;
			this.MyAjax(0, url, suc, function () {
				that.Tip("网络错误");
			}, data, noLoad);
		},
		//同get请尔
		MyPost: function (url, suc, data, noLoad) {
			var that = this;
			this.MyAjax(1, url, suc, function () {
				that.Tip("网络错误");
			}, data, noLoad);
		},
		...
		IsLast: function (dom, height, fuc) {
			...
			scroll(function (direction) {
				if (direction === 'down' && fuc) {
					setTimeout(function () {
						if (d.scrollHeight - d.scrollTop < domH + h) {
							fuc();
						}
					}, 200);
				}
		...
		topRef: function (id, ref) {
			$("body").append('<div class="top-ref" id="top-ref"><span>&nbsp;</span></div>');
			var F = $(id);
			var L = {
					ts: false,
					s: 0,
			...
			function b() {
				M.attr("style", false);
				N.attr("style", false);
				M.removeClass("top_anm");
				L.m = false;
			}

			function c() {
				M.attr("style", "opacity:0.6;transform:translate(0,100%);-webkit-transform:translate(0,100%);");
				N.attr("style", "transform:rotate(0deg);-webkit-transform:rotate(0deg);");
				L.ts = false;
				L.s = -10;
				setTimeout(function () {
					M.addClass("top_anm");
			...
			F.on("touchstart", function (a) {
				...
				if (0 !== b) {
					...
					L.ts = a.targetTouches[0];
					M.removeClass("top_anm");

				}
			});
			F.on("touchmove", function (a) {
				if (L.ts && a.targetTouches[0].pageY - L.ts.pageY > 0) {
					if (L.mk = Math.abs(a.targetTouches[0].pageX - L.ts.pageX) > (a.targetTouches[0].pageY - L.ts.pageY) / 2, L.s = a.targetTouches[0].pageY - L.ts.pageY, L.mk || L.s < 30) {
						L.s = -10;
						b();
						return false;
					}
					L.s -= 30;
					if (L.s < 150) {
						M.attr("style", "opacity:" + (L.s / 149 + L.op) + ";transform:translate(0," + (3 * L.s - L.s_n) + "%);-webkit-transform:translate(0," + (3 * L.s - L.s_n) + "%);transition:none;-webkit-transition:none;");

					} else {
						N.attr("style", "transform:rotate(" + 7.2 * L.s + "deg);-webkit-transform:rotate(" + 7.2 * L.s + "deg);transition:none;-webkit-transition:none;");
					}
				}
			});
...
(function () {
	...
	var ms = {
		cc: function () {
			init.PopupFuc.cc(init);
		},
		cf: function () {
			init.PopupFuc.cf(init);
		}
	};
	init.TipString = init.V('#vue-bom-warp', {
		tip: '网络错误',
		selecTip: '你确定要 吗？',
		dom: false
	}, ms);
